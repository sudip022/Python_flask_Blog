...
class Posts(db.Model):
    ...
    tagline = db.Column(db.String(15),  nullable=False)
    date = db.Column(db.String(12), nullable=True)
...
def dashboard():
    ...
    if request.method == 'POST':
        ...
        if (username == params['admin-user'] and userpass == params['admin_password']):
...
def contact():
    if(request.method == 'POST'):
        ...
        entry = Contacts(name=name, phone_num=phone,
                         date=datetime.now(), mes=message, email=email)
    ...
    if request.method == 'POST':
        ...
        if sno == "0":
            post = Posts(title=box_title, slug=slug,
                         content=content, tagline=tline, date=date, img_file=img_file)
            db.session.add(post)
            db.session.commit()
